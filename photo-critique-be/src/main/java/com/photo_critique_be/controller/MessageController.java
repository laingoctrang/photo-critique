...
public class MessageController {
    ...
    public ResponseEntity<ApiResponse<PageResponse<UserListItemResponse>>> searchUsersForChat(
            @RequestParam String query,
            @PageableDefault(size = 20) Pageable pageable) {
        PageResponse<UserListItemResponse> users = messageService.searchUsersForChat(query, pageable);
